import React from "react";
import axios from "axios";
import { connect } from "react-redux";
import { updateLeavingCertificateItem } from "../../redux/actions/studentSection";
const LCRequestItem = ({
  studentsection: { leavingCertificateRequests },
  updateLeavingCertificateItem,
  mis,
  dept,
  name,
  academicYear,
  programme,
  remark,
  status,
}) => {
  const imageUrl =
    "data:image/jpeg;base64,";

  const updateItem = async () => {
    const data = {
      name,
      mis,
      dept,
      academicYear,
      programme,
      remark,
      status: "Approved",
    };
    updateLeavingCertificateItem(data);
  };
  return (
    <li className="py-3 sm:py-4 hover:bg-gray-100 p-3 hover:rounded-xl">
      <div className="flex items-center">
        <div className="flex-shrink-0 pr-5">
          <img
            className="w-8 h-8 rounded-full"
            src={imageUrl}
            alt="Neil image"
          />
        </div>
        <div className="flex-1 min-w-0 ms-4">
          <p className="text-md font-medium text-gray-900 truncate">
            {mis + " " + name}
          </p>
          <p className="text-md text-gray-500 truncate">{dept}</p>
          {status === "notApproved" ? (
            <div>
              <button
                className="m-1 text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800"
                onClick={updateItem}
              >
                Approve
              </button>
              <button className="m-1 focus:outline-none text-white bg-red-700 hover:bg-red-800 focus:ring-4 focus:ring-red-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 dark:bg-red-600 dark:hover:bg-red-700 dark:focus:ring-red-900">
                Reject
              </button>
            </div>
          ) : (
            <div
              className="w-24 p-2 mb-1 mt-1 text-sm text-green-800 rounded-full bg-green-50 "
              role="alert"
            >
              <span className="ml-1 font-medium">Approved</span>
            </div>
          )}
        </div>
        <div className="inline-flex flex-col items-center text-base font-semibold text-gray-900">
          {academicYear}
          <span className="inline-flex items-center text-sm font-normal text-gray-500 p-3 ">
            {programme}
          </span>
        </div>
      </div>
    </li>
  );
};

const mapStateToProps = (state) => ({
  studentsection: state.studentsection,
});

export default connect(mapStateToProps, { updateLeavingCertificateItem })(
  LCRequestItem
);
