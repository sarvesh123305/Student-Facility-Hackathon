...
import BonafideRequestItem from "./BonafideRequestItem";
import FilterDropdown from "../Student/FilterDropdown";

const BonafideRequests = ({
  studentsection: { bonafideRequests },
  initialLoadUser,
  getBonafideRequests,
}) => {
  useEffect(() => {
  ...
  const [data, setData] = useState(bonafideRequests);
    ...
    <div className="w-full p-4 bg-white border-gray-200 rounded-lg shadow sm:p-8">
      ...
      <div className="flow-root">
        ...
        {selectedValue === 'All' && data && data.length > 0 && data.map(item => (
            <BonafideRequestItem key={item._id} mis={item.mis} name={item.name} remark={item.remark} programme={item.programme} ay={item.academicYear} dept={item.dept} status={item.status} render={setData} />
          ))}
